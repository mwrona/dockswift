...
def get_temp_url(request, storage_url, auth_token, container, objectname, expires=600):
    ...
    if request.is_secure():
        host_url = "https://%s" % request.get_host().split(":")[0].lower()
    else:
        host_url = "http://%s" % request.get_host().split(":")[0].lower()
    url = '%s:8080%s?temp_url_sig=%s&temp_url_expires=%s' % (
        host_url, path, sig, expires)
    return url
